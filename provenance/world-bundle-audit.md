...
`mountFoundationBrowser` and `@herdr-world/foundation/styles.css`; it does not
...
and World assembly in `web/dist/assets/main-H7RFRp6L.js` (895,790 bytes), the
Foundation terminal chunk in `ghostty-web-0K3DfDqq-CdDHdjG2.js` (636,560
bytes), and the combined stylesheet in `main-Dgw-HtQ4.css` (185,580 bytes).
...
- generic rules co-located with World rules in `web/src/styles.css`
...
owner, or Foundation-private imports. The old handoff/settings files remain
only for deferred removal and legacy tests; they are not imported by the
selected Foundation assembly.
