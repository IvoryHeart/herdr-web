...
The optional `HERDR_SRC` mode exact-compares every copied source file and the protocol-20 markers.
The default mode verifies the manifest's destination hashes, required protocol-20 wire shapes, and
crate layout. Locally adapted files are intentionally excluded from exact comparison and must be
reviewed manually during refresh. `PopupSize` is compared with only the documented visibility
adaptation allowed. Frozen protocol frames live in `vendor/herdr-compat/tests/` and are tested by
`protocol20_fixtures.rs`.
