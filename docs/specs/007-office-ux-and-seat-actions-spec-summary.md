...
- **Evidence:** `npm run test:web` — 48 files / 329 tests passed; `npm run lint:web` passed; `npm run build:web` passed with the existing large-chunk warning. The targeted browser regression confirms that an admitted new seat opens automatically while an existing Office terminal remains open. The full World browser suite still has the pre-existing room-callout hover failure in this environment; that baseline failure is separate from the lifecycle changes here.
