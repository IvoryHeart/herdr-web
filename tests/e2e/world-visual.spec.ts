...
const evidenceDir = resolve("docs/evidence/spec-010-extension-001");
...
for (const viewport of [
  ...
  test(`captures deterministic World at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    await expect(page.locator(".brand-sub")).toHaveText("1 blocked");
...
async function waitForLiveOffice(page: import("@playwright/test").Page) {
  await expect(page.locator(".world-agent-row").filter({ hasText: "Codex A" })).toBeAttached();
  await expect(page.locator(".world-agent-row").filter({ hasText: "Codex B" })).toBeAttached();
}
